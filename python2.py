...
def extract_ocr_text(page):
    ...
    try:
        if not ocr_button.get_attribute("aria-expanded") or ocr_button.get_attribute("aria-expanded") == "false":
                ocr_button.click()
                time.sleep(2)  # Allow time for text to load

        # ✅ Wait for OCR container to appear
        print("🔍 Waiting for OCR text container to appear...")
        page.wait_for_selector("div#mCSB_13_container", state="attached", timeout=8000)
    ...
    for attempt in range(3):  # Try extracting metadata up to 3 times
        ...
        if about_container:
            ...
            for key, search_text in key_mappings.items():
                ...
                if label_element:
                    # ✅ Look for the next `<span>` or `<div>` after `<b>` tag
            ...
            if metadata["Title"] != "Not Found" and metadata["Publisher"] != "Not Found":
        ...
        while True:
            ...
            if len(next_buttons) > 0:
                ...
                if next_button.is_visible():
                    ...
                    next_button.click()
                    page.wait_for_load_state("networkidle")
                    time.sleep(3)  # Allow time for the new results to load
                else:
